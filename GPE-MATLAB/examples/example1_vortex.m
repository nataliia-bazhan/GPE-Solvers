...
N=128; % number of grid points for coordinate grid
Nz=16;
grid = grid3d(xmax,N,xmax,N,xmax,Nz); % create a two-dimensional NxN grid object

V = @(X,Y,Z) 0.5*(X.^2 + Y.^2); % function representing the external potential (must have 3 arguments)
task = ZNGtask(grid,V); % initialize the GPE solver
